...
# Support both package-relative and top-level imports for testing. When
# imported as part of the ``rise_pipeline`` package the relative imports
# will succeed. When this module is executed or imported as a
# top-level script (e.g. ``import scoring`` from within the same
# directory), the relative imports will fail. In that case we fall back
# to importing the modules from the current working directory.
try:
    from .models import (
...
try:
    from .config import PAIN_SCORE_REJECT_THRESHOLD, PAIN_SCORE_MARGINAL_THRESHOLD  # type: ignore
except ImportError:
    from config import PAIN_SCORE_REJECT_THRESHOLD, PAIN_SCORE_MARGINAL_THRESHOLD  # type: ignore
