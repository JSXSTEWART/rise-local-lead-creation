...
# Attempt to import load_dotenv from the python-dotenv package. In
# environments where the dependency is unavailable (e.g. offline tests),
# fall back to a no-op implementation. This ensures that importing this
# module does not raise ModuleNotFoundError.
try:
    from dotenv import load_dotenv  # type: ignore
except ImportError:
    def load_dotenv(path: str = None, *args, **kwargs):  # type: ignore
        """Fallback no-op load_dotenv.

        This stub accepts the same signature as python-dotenv's load_dotenv
        but simply returns without doing anything. It allows the
        configuration module to be imported even when python-dotenv is
        not installed.
